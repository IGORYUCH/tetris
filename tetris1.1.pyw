...
def animate():# функция, отвечающая за отрисовку всех элементов игры
    ...
    score_text = text_font.render('score:{: >6}'.format(int(score)),0,TEXT_COLOR)
...
try:
    ...
    GREED = TEXR_COLOR if conf_data['GRID_ON'] else AIR_COLOR
...
hint_text = hint_font.render('right/left - move  up - rotate  down - boost speed  r_ctrl - swap figure  r - restart',0,TEXT_COLOR)
while game_alive:
    if not losed:
